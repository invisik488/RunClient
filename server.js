...
const { Pool } = require('pg'); // подключаем драйвер PostgreSQL
const app = express();
...
const pool = new Pool({
  ...
  ssl: { rejectUnauthorized: false } // отключает проверку сертификата
});
...
app.post('/api/search', async (req, res) => {
  ...
  try {
    // Ищем пользователя в таблице users (регистронезависимо)
...
app.post('/api/users/:id/role', async (req, res) => {
  const { id } = req.params;
  const { role } = req.body; // например, 'admin' или 'user'
